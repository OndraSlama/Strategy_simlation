...
void StrategyB::CalculateDesiredAxisPositions()
{
	int currentDist;
	int minDist;
	bool foundBest;
...
void StrategyB::Attack(int index)
{
...
void StrategyB::BackwardAttack(int index)
{
	// osy pred micem brani, s micem strili, pred micem zvednou
...
void StrategyB::CameraInput(int x, int y)
{
...
void StrategyB::SetBall(double x, double y)
{
	if (x == 0 && y == 0)
	{
...
// unused
void StrategyB::PyramidDefense(int index)
{
	int pyrLayer = 0;
	int minDist;
	int currentDist;
